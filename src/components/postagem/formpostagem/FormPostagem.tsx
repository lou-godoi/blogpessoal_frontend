  ...
  async function gerarNovaPostagem(e: FormEvent<HTMLFormElement>) {
    ...
    if (id !== undefined) {
      try {
        ...
        ToastAlerta("Postagem atualizada com sucesso", "sucesso"); // Toast de sucesso
        retornar();
      } catch (error: any) {
        if (error.toString().includes("401")) {
          handleLogout();
        } else {
          ToastAlerta("Erro ao atualizar a Postagem", "erro"); // Toast de erro
        }
      }
    } else {
      try {
         ...
         <div className="relative z-10 w-full max-w-xl px-4">
          <div className="bg-stone-900/80 backdrop-blur-md border border-emerald-500/20 rounded-[2rem] p-8 shadow-2xl">
              ...
              <form onSubmit={gerarNovaPostagem} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <label htmlFor="texto" className="text-emerald-400 font-bold uppercase tracking-widest text-[10px] ml-2">Conteúdo do Pergaminho</label>
                    <textarea
                  ...
                  <div className="flex flex-col gap-1">
                    <label htmlFor="tema" className="text-emerald-400 font-bold uppercase tracking-widest text-[10px] ml-2">Escolha a Categoria</label>
                    <select
                      name="tema"
                      id="tema"
                    ...
                    >
                      ...
                      {temas.map((tema) => (
                        <option key={tema.id} value={tema.id} className="bg-stone-900 text-emerald-100">
                          {tema.descricao}
                        </option>
                      ))}
                    </select>
